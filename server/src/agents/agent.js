export class Agent {
  ...
  async onBroadcast(history, lastMessage) {
    ...
    this.broadcaster.io.emit('typing_start', { agentId: this.id, displayName: this.displayName });

    try {
      const result = await this.provider.generate({ persona: this, history, lastMessage });
      if (result?.noResponse || !result?.content) {
        this.broadcaster.io.emit('typing_stop', { agentId: this.id });
